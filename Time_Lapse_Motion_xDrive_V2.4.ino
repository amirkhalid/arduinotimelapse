/*  
version updates:
2.4 added executable utk menu item2 - 1m slide left
2.3 integrate Timelapse program with LCD Menu by Paul S - test with one menu 1m Right. 
    SUCCESS
    to change btnRight used to select item to btnSELECT
2.2 tidy up display text, fine tune speed and length
2.1 to slide 1 meter to right (CW) with 1 variable - time is user input dependent - SUCCESS
...
Component:                      
Arduino UNO
LCD Keypad Shield ver1.0
DVR8225 motor driver
Wantaimotor geared 1:5.18 stepper, 12V 0.4A
...
B2      > Coil B-
    ...
    **** Timelapse slider program ****
    ...
    Program aim is to move either 1m or 2m, in selected direction, in set time ("runtime")so speed is automatically calculated
    
    The step/flow will be:
    ## Welcome Screen (5 seconds)##
    
    1. Adjust carrier position if necessary, move left or right to proper position (mark on slider?)
    2. Select length, 1m or 2m
    3. Select direction, right->left or left->right (+ve /-ve steps) 
        NOTE : btn LEFT = SLIDE LEFT = -ve STEPS/CCW 
               btn RIGHT = SLIDE RIGHT = +ve STEPS/CW 
    4. Set slide duration or runtime in multiply of 5 minutes (fine tune of 1 minute necessary)
    5.
      a Execute program - slide 1M / 2M, left/right in xx minutes
      b Display while program run : remaining time to completion
    6.  Turn motor power off upon completion to save battery, Flash notification upon completion
*/
...
// You can have up to 10 menu items in the menuItems[] array below without having to change the base programming at all. Name them however you'd like. 
